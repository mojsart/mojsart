...
var readDirEach = function(func) {
  Q.nfcall(fs.readdir, dirName)
    .then(function(files) {
      async.each(files, func, callbackError);
    })
    .fail(callbackError);
};
...
var echoFetchMD5 = function(md5, bool, filename, interval) {
  echo.get(md5, function(json) {
    ...
    if (json.response.track.status === "complete") {
      updateSong(json.response.track, filename);
      if (bool === false) {
        bool = true;
      }
    }
...
var saveSong = function(trackData, filename) {
  var song = new Song({
    echoData: {
...
var updateSong = function(trackData, filename) {
  var update = {
    echoData: {
...
var checkSongNotInDB = function(searchField, input, notfoundcb, foundcb) {
...
var postSongSave = function(fromPath, toPath, cb) {
...
module.exports = exports = {
  dirName           : dirName, 
  readDirEach       : readDirEach,
  echoFetchMD5      : echoFetchMD5,
  saveSong          : saveSong, 
  updateSong        : updateSong,
  checkSongNotInDB  : checkSongNotInDB,
  callbackError     : callbackError,
  filenameRegEx     : filenameRegEx,
  filesizeCheck     : filesizeCheck, 
  postSongSave      : postSongSave
};
