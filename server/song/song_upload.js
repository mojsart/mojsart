...
var uploadSongs = function() {
  ...
  helpers.readDirEach(validateFileType);
};

// validate songtype using regex filter and check if filename is found in db
var validateFileType = function(file, callback) {
  console.log('validating filetype');
  if(helpers.filenameRegEx(file)) {
    console.log('processing file', file);
    helpers.checkSongNotInDB('filename', file, echoUpload, recacheExistingSong);
  }
};

var recacheExistingSong = function(filename) {
...
var echoUpload = function(filename, callback) {
  ...
  .then(function(buffer) {
    ...
    Q.nfcall(echo.postBuffer, buffer)
      .then(function(echoNestResponse) {
        ...
        handleEchoResponse(null, p, filename);
      });
  });
};
...
var handleEchoResponse = function(err, json, filename) {
  ...
  helpers.checkSongNotInDB('filename', filename, function(filename) { 
    ...
    helpers.checkSongNotInDB('echoData.md5', json.response.track.md5, function(md5) {
      setEchoInterval(md5, filename);
    });
  });
};

// ping echo nest for status update
var setEchoInterval = function(md5, filename) {
  var waittime = 5000;
...
module.exports = exports = {
  ...
  recacheExistingSong : recacheExistingSong,
  echoUpload          : echoUpload,
  handleEchoResponse  : handleEchoResponse,
  setEchoInterval     : setEchoInterval
};
